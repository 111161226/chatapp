...
func RegisterUser(ctx *gin.Context) {
	...
	switch {
	case username == "":
		ctx.HTML(http.StatusBadRequest, "signup.html", gin.H{"Error": "Usernane is not provided", "Username": username})
		return
	case password == "":
		ctx.HTML(http.StatusBadRequest, "signup.html", gin.H{"Error": "Password is not provided", "Password": password})
		return
	case pass_con == "":
		ctx.HTML(http.StatusBadRequest, "signup.html", gin.H{"Error": "Confirmed Password is not provided",
			"Pass_con": pass_con})
		return
	}
	...
	case password != pass_con:
		ctx.HTML(http.StatusBadRequest, "signup.html", gin.H{"Error": "Confirmed Password doesn't match Password", "Username": username, "Password": password, "Pass_con": pass_con})
		return
	case len(password) < en_len:
		ctx.HTML(http.StatusBadRequest, "signup.html", gin.H{"Error": "Password doesn't have enough length", "Username": username, "Password": password, "Pass_con": pass_con})
		return
	default:
		//check security
		upperA := rune('A')
		...
		if !(num && big && small) {
			ctx.HTML(http.StatusBadRequest, "signup.html", gin.H{"Error": "Password doesn't include all these(number, uppercase and lowercase letter)", "Username": username, "Password": password, "Pass_con": pass_con})
			return
		}
	}
	...
	if duplicate > 0 {
		ctx.HTML(http.StatusBadRequest, "signup.html", gin.H{"Error": "Username is already taken", "Username": username, "Password": password, "Pass_con": pass_con})
		return
	}
	...
	tx.Commit()
	ctx.Redirect(http.StatusFound, "/openchat")
}
